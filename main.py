...
def run_inference_sample():
    ...
    for sample_question, sample_answer, sample_comp_level in tqdm(zip(questions, answers, complexity_levels)):
        ...
        if str(sample_answer) in response:
            correct += 1
            if sample_comp_level == 1: easy+=1
            elif sample_comp_level == 2: medium+=1
            else: hard += 1
        else:
            print("\nResponse: ", response, "\nGround Truth: ", sample_answer, "\n")
        if sample_comp_level == 1: easy_total+=1
        elif sample_comp_level == 2: medium_total+=1
        else: hard_total += 1

    print("\nAccuracy: ", correct / len(questions))
    print("\nAccuracy on Easy: ", easy/easy_total, "Easy correctly classified: ", easy, "Total correct: ", easy_total)
    print("\nAccuracy on Medium: ", medium/medium_total, "Medium correctly classified: ", medium, "Total medium: ", medium_total)
    print("\nAccuracy on Hard: ", hard/hard_total, "Hard correctly classified: ", hard, "Total hard: ", hard_total)
